...
Model::~Model()
{
    ...
    {
        texture_loaded[i].TextureRelease();
    }
}
...
vector<Texture> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            if (std::strcmp(texture_loaded[j].GetPath().data(), (this->m_directory + '/' + str.C_Str()).data()) == 0)
            {
        ...
        {   // if texture hasn't been loaded already, load it
            string temp_path = this->m_directory + '/' + str.C_Str();
            Texture texture(temp_path, typeName,true);
